...
map.hasIndexes = function(indexes = [])
	for index in indexes
		if not self.hasIndex(index) then return false
	end for
	return true
end function
...
map.last = function
	if self.len < 1 then return null
	return {self.indexes[self.indexes.len-1]:self[self.indexes[self.indexes.len-1]]}
end function
string.color = function(c)
	...
	if not @c isa funcRef then return ""
	colored = ""
